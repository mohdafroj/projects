...
| Translator EN→HI | `TRN-EN-001` | `trans123` |
| Translator HI→EN | `TRN-HI-001` | `trans123` |
| JS | `JS-001` | `js123` |
| SG | `SG-001` | `sg123` |
...
5. Optional Translator segment: login as `TRN-EN-001`, open `/translator/queue`, open the demo assignment, request AI assist, accept or edit one AI suggestion, and commit. Explain that commit seals translator intent and forwards to Reviewer/Vetter.
6. JS: open `/js/queue`, accept one suggested edit, forward the window to SG.
7. SG: open `/sg/tray`, open a window, confirm expunge candidates, sign with the DSC stub.
8. Director: open `/director/inbox`, publish the queued job, confirm CRC preview and Digital Sansad push.
9. Admin: open `/admin/system-health` to confirm Laravel, Postgres, Redis, frontend, ML gateway, realtime sidecar, and Meilisearch are healthy.
