...
<head>
    ...
    <title>Vanisetu · वाणीसेतु · Listener</title>
...
<body>
    ...
    <div
        ...
        data-target-languages="{{ implode(',', $listenerTargetLanguages ?? [$targetLanguage]) }}"
        data-requested-target-language="{{ $requestedTargetLanguage ?? $targetLanguage }}"
        data-fallback-applied="{{ !empty($listenerFallbackApplied) ? 'true' : 'false' }}"
    >
        @foreach ($sessions as $session)
            @foreach ($session->outputs as $output)
                @php
                    $audioPath = $output->audio_output_path;
                    $audioUrl = filled($audioPath)
                        ? (str_starts_with($audioPath, 'http://') || str_starts_with($audioPath, 'https://') || str_starts_with($audioPath, '/')
                            ? $audioPath
                            : (str_starts_with($audioPath, 'storage/') ? '/'.$audioPath : null))
                        : null;
                @endphp
                {{-- Ephemeral: no transcript/translation text is displayed. --}}
                @if ($audioUrl)
