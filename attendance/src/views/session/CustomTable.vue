      ...
      <table class="min-w-full text-sm table-fixed">
        <thead>
          <tr>
            <th v-for="col in columnMeta" :key="col.accessorKey"
              class="px-5 py-4 font-semibold text-center align-middle bg-blue-100">
              <div class="flex items-center gap-1 cursor-pointer"
                @click="sortByColumn(col.header, filters.sort.order)">
                <span>{{ formatHeader(col.header) }}</span>
             ...
             <th v-if="columnMeta.length != []" class="px-5 py-4 font-semibold bg-blue-100 text-center align-middle">
              Actions
            </th> 
          </tr>
        </thead>
        ...
        <tbody class="bg-white">
          <tr v-for="(row, idx) in tableData" :key="idx" class="relative border-t align-middle " :class="[props.showOpen ? 'hover:bg-gray-100' : '']"
            ...
            <td v-if="$slots['cell-before']" class="px-5 py-4 text-center align-middle">
              <slot name="cell-before" :row="row" :index="row.id" />
            </td>

            <td v-for="col in columnMeta" :key="col.accessorKey" class="px-5 py-4 text-center align-middle">
