    ...
    <div class="pb-8 px-2 h-[400px] overflow-y-auto">
      ...
      <div class="grid grid-cols-3 gap-4 mt-4">
        <!-- <div>
          ...
          />
          <span v-if="formErrors.start_date" class="text-sm text-red-500">{{ formErrors.start_date }}</span>
        </div>
        ...
        <div>
          <TextInput
            ...
            type="date"
            label="End Date"
            @update:modelValue="(data) => handleFormData(data, 'toDate')"
          />
          <span v-if="formErrors.end_date" class="text-sm text-red-500">{{ formErrors.end_date }}</span>
        </div> -->
        <div>
          <label class="inline-block input-label mb-[6px] text-gray-500 text-sm font-semibold dark:text-slate-300">Start Date *</label>
          <input type="date" 
          :min="minDate" 
          :max="maxDate" 
        ...
        <div>
          <label class="inline-block input-label mb-[6px] text-gray-500 text-sm font-semibold dark:text-slate-300">End Date *</label>
          <input type="date" 
          :min="minDate" 
          :max="maxDate" 
          v-model="formData.end_date"
          ref="inputMaxDateRef"
          @input="(data) => handleFormData(data, 'toDate')"
          @click="() => handleInputDateClick('maxDate')" 
          class="cursor-pointer border rounded-lg px-3 py-2 w-full" />
          <span v-if="formErrors.end_date" class="text-sm text-red-500">{{ formErrors.end_date }}</span>
        </div>
        <div>
          <SelectInput type="select" label="Leave Reason" v-model="select_reason" :options="reasons" @change="(data) => handleFormData(data, 'selectreason')" />
        </div>            
      </div>
...
import { Button, FileUploads, SelectInput, TextArea } from "@sds/oneui-common-ui";
import { fetchActiveMemberList, addLeaveRequest } from "@/services/attendanceService";
import { getSessionData } from "@/services/rss/sessionService";
...
const formInstance = { session_id: props.sessionDetails.id, member_id: '', start_date: today, end_date: today, file: '', reason: '' };
const select_reason = ref('');
...
const inputMaxDateRef = ref(null);
const sittingDates = ref([]);
const maxDate = computed(() => {
  let d = null;
  if ( props.sessionDetails.session_end_date ) {
    d = new Date(props.sessionDetails.session_end_date)
  } else {
    d = new Date();
...
const handleInputDateClick = (param) => {
  ...
  } else if ( param == 'maxDate' ) {
    inputMaxDateRef.value.showPicker();
  }
}
...
const handleFormData = (data, fieldName) => {
  ...
  if (fieldName === 'fromDate') {
    formData.value.start_date = data.target.value;
    formErrors.value.start_date = '';
  } else if (fieldName === 'toDate') {
    formData.value.end_date = data.target.value;
...
const validate = () => {
  ...
  if (!formData.value.start_date) errors.start_date = 'The field from date is required.';
  if (!formData.value.end_date) errors.end_date = 'The field to date is required.';
  ...
  if (formData.value.start_date && formData.value.end_date) {
    const from = new Date(formData.value.start_date);
    const to = new Date(formData.value.end_date);
    const checkMinDate = new Date(minDate.value); 
    const checkMaxDate = new Date(maxDate.value);
    if ( !isNaN(checkMinDate) && !isNaN(from) && from < checkMinDate ) {
      const yyyy = checkMinDate.getFullYear();
      const mm = String(checkMinDate.getMonth() + 1).padStart(2, "0");
      const dd = String(checkMinDate.getDate()).padStart(2, "0");
      errors.start_date = `The date must be greater than or equal to ${yyyy}-${mm}-${dd}`;
    }
    if (!isNaN(checkMaxDate) && !isNaN(to) && checkMaxDate < to) {
      ...
      errors.end_date = `The date must be less than or equal to ${yyyy}-${mm}-${dd}`;
    } else if (!isNaN(from) && !isNaN(to) && from > to) {
      errors.end_date = 'To date must be the same or after From date.';
    }
  }
...
const handleSubmit = async () => {  
  ...
  Swal.fire({
    ...
    title: (response.isError == false && response.success_code == 200) ? 'Leave Request added successfully' : response.customMessage,
...
onMounted( async () => {
  // const response = await getSessionData(props.sessionDetails.id);
  // if (response.success_code == 200) {
  //   sittingDates.value = response.data.sitting_dates;
  // }
