  ...
  <div>
   ...
   <!-- <Loading v-if="isPdfLoading" /> -->
    ...
    <div class="mx-auto p-0">
      <PageLoading v-show="loadingData && !parentLoading" customClasses="absolute top-auto bottom-0" />
      <!-- Table Options -->
      <TableOptions :tableTitle="'Attendance Report'" :columns="columns" :data="[]"
...
<div>
  ...
  <div class="mt-6 flex justify-start">
    <Button 
    v-if="hasAnyPermission([PERMISSIONS.ATTENDANCE.FINALIZE_ATTENDANCE])" 
    label="Finalize Attendance" 
    icon="ri:checkbox-fill" 
    size="sm" 
    class="bg-red-800 hover:bg-red-800"
    :disabled="!isNight"
...
/> -->
  ...
  <div v-if="errorMsg" class="text-red-600 mt-4 text-center font-semibold">
     ...
     <div v-if="isLoading"
      class="fixed inset-0 h-screen w-screen z-[9999] flex items-center justify-center bg-white dark:bg-slate-900"
    >
...
import { getFilterReportDashboard, getReportDashboard, downloadReport, updateEsign,downloadsignedFile,resendNotificationApi, msaDownloadReport, publishReportDashboard, toFinalizeAteendace  } from '@/services/attendanceService'
import PdfModal from '../PdfModal.vue'
import ShareEmailModal from '../ShareEmailModal.vue'
import { useApiStore } from "@/store/apiData";
import { PERMISSIONS, hasAnyPermission } from '@/utils/rbac'
...
const resendApi = async (media_id) => {
  try {
    //console.log("resresend", media_id);
    isReset.value= true;
    const resresend = await resendNotificationApi(media_id); 
    ...
    try {
      ...
      if (resEsign.success_code == 200 && resEsign.data != '') {
       ...
       // Proceed with download or next step
          ...
          // link.href = url;
          // link.target = '_blank'; 
          // link.setAttribute('download', 'document.pdf'); // Suggested filename
          // document.body.appendChild(link);
          // link.click();
...
watch(() => props.date, async (newDate) => {
  ...
  await attendanceReportlist();
  await checkReportAvailability(); // msa download btn
  fetchFilterReportDashboard();
  publishedData.value = props.isPublished == 1;
}, { immediate: true }); 
...
const finalizeAttendance = async () => {
  ...
  const response = await toFinalizeAteendace(options);
  loadingData.value = false;
  Swal.fire({
    toast: true,
    position: 'top-end',
    timer: 5000,
    timerProgressBar: true,
    icon: (!response.isError && response.success_code == 200) ? "success": "error",
    text: (!response.isError && response.success_code == 200) ? "We have successfully finalized the attendance.": response?.customMessage,
    showConfirmButton: false
  });
}
//End of Finalize Attendance
