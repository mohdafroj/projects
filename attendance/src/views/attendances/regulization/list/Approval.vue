    ...
    <div v-else>
      <div class="bg-gray-50 dark:bg-gray-800 min-h-screen p-8 rounded-xl">
        ...
        <div class="grid grid-cols-2 gap-4 mb-5">
          ...
          <div class="flex justify-end items-start gap-2 mt-4">
            ...
            />
            <Button
              v-if="module?.action && hasAnyPermission([PERMISSIONS.APPROVAL.LEAVE.INITIATE, PERMISSIONS.APPROVAL.LEAVE.REVIEW])"
              label="Send"
              color="blue-outline"
              style="padding: 2px 10px; border-radius: 5px"
        ...
        <div class="grid grid-cols-2 gap-6">
          ...
          >
            <div v-if="Object.keys(itemsData).length" class="mb-4">
              <div class="flex items-start w-full">
                <div class="w-[70%]">
                  <p class="font-semibold text-gray-700">{{ itemsData?.name || '' }}</p>
                  <p class="text-sm text-gray-500">Request Id: {{ itemsData?.request_id || '' }}</p>
                  <p class="text-sm text-gray-500">Leave from: {{ itemsData?.start_date ? useLocalDate(itemsData?.start_date, 'dd-mm-yyyy') : '' }} to {{ itemsData?.end_date ? useLocalDate(itemsData?.end_date, 'dd-mm-yyyy') : '' }}</p>
                  <p class="text-sm text-gray-500">Total Leave: {{ itemsData?.total_days || '0' }} {{ itemsData?.total_days > 1 ? 'Days' : 'Day' }}</p>
                  <p class="text-sm text-gray-500">Applied On: {{ itemsData?.created_at ? useLocalDate(itemsData?.created_at, 'dd-mm-yyyy') : '' }}</p>
                </div>
                <img v-if="itemsData?.profile_photo" :src="itemsData?.profile_photo" class="ml-auto w-[30%] object-contain" />
              </div>
              <hr class="mt-1" />
              <div v-if="module?.module?.file_number">
                <p class="font-semibold text-gray-700">e-Office File Number: {{ module.module.file_number || '' }}</p>
                <p class="text-sm text-gray-500">File Date: {{ module.module?.file_date ? useLocalDate(module.module?.file_date, 'dd-mm-yyyy') : '' }}</p>
                <p class="text-sm text-gray-500">Remark: <span v-html="module.module?.approval_rermark"></span></p>
                <p class="text-sm text-gray-500">Reason: <span v-html="module.module?.short_reson"></span></p>
                <hr class="mt-1" />
              </div>
            </div>
      ...
      >
        <div class="p-5">
          ...
          <div v-show="hasPermission(PERMISSIONS.APPROVAL.LEAVE.INITIATE)">
            <div class="mb-4">
              Upload File:
              <label
              for="fileUpload"
            ...
            <div class="mb-4 grid grid-cols-2 gap-4">
              <div>
                <SelectInput type="select" label="Leave Reason" :is-required="true" v-model="short_remark_select" :options="reasonOptions" @change="handleShortRemark" />
              </div>
              <div v-show="short_remark_select == 'Other'">
        ...
        <div class="p-5">
          ...
          <div v-show="false">
            ...
            <div class="mb-4 grid grid-cols-2 gap-4">
              <div>
                <TextInput
                  v-model="approvalForm.file_number"
                  type="text"
                  :is-required="true"
                  label="e-Office File Number"
                  placeholder="Enter e-office file number"
              ...
              <div>
                <TextInput
                  v-model="approvalForm.file_date"
                  :is-required="true"
                  type="date"
                  label="File Approval Date"
                />
...
import { fetchReasons } from '@/services/attendanceService';
...
watch(
  [module, reasonOptions],
  ([newModule, newReasonOptions]) => {
    if (!newReasonOptions?.length) return;

    const mod = newModule?.module ?? {};
    const reason = mod.short_reson || '';

    // reset first
    short_remark_select.value = '';
    short_remark_text.value = '';

    if (reason) {
      const matched = newReasonOptions.find(
        item =>
          item.type === 'LEAVE' &&
          item.value === reason &&
          reason !== 'Other'
      ...
      if (matched) {
        short_remark_select.value = reason;
      } else {
        short_remark_select.value = 'Other';
        short_remark_text.value = reason;
      }
    }

    if ( hasPermission(PERMISSIONS.APPROVAL.LEAVE.APPROVE) ) {
      approvalForm.file_number = mod.file_number || '';
      approvalForm.file_date = mod.file_date || '';
      approvalForm.remarks = mod.approval_rermark || '';
    } else {
      form.file_number = mod.file_number || '';
      form.file_date = mod.file_date || '';
      form.remarks = mod.approval_rermark || '';
    }
  },
  { immediate: true }
);
...
onMounted( async() => {
  ...
  itemsData.value = sessionStorage.getItem('LeaveApproveItem' + module_id) ? JSON.parse(sessionStorage.getItem('LeaveApproveItem' + module_id)) : {};
}) 
...
const sendToReviewUser = () => {
  ...
  let payload = {
    ...
    let file = '';
    const fileData = uploadedFiles.value.length ? uploadedFiles.value[0] : [];
    if ( fileData?.errors?.length ) {
      fireToast({ type: 'error', message:  fileData.errors[0] });
      return false;
    } else {
      file = fileData['path'] || '';
    }
    ...
    if (!form.file_number) {
      fireToast({ type: 'error', message: 'Please enter e-Office file number' });
      return false;
    } else if ( !/^[a-zA-Z0-9]+$/.test(form.file_number.trim()) ) {
      fireToast({ type: 'error', message: 'e-Office file number should be alpha numeric only.' });
      return false;
    }

    if (!form.file_date) {
      fireToast({ type: 'error', message: 'Please choose file date.' });
      return false;
    }

    const remarks = form.remarks ? form.remarks.replace(/<\/?[^>]+(>|$)/g, "").replace(/&nbsp;/gi, "").trim() : form.remarks;
    if (!remarks) {
      fireToast({ type: 'error', message: 'Please enter the remark.' });
      return false;
    }

    let shortRemark = '';
    if ( !short_remark_select.value ) {
      fireToast({ type: 'error', message: 'Please select a reason.' });
      return false;
    } else if (short_remark_select.value == 'Other' && short_remark_text.value.trim() == '') {
      fireToast({ type: 'error', message: 'Please enter short remark' });
      return false;
    } else {
...
const approvedSubmit = async () => {
  ...
  let file = '';
    ...
    if (!approvalForm.file_number) {
      fireToast({ type: 'error', message: 'Please enter e-Office file number' });
      return false;
    } else if ( !/^[a-zA-Z0-9]+$/.test(approvalForm.file_number.trim()) ) {
      fireToast({ type: 'error', message: 'e-Office file number should be alpha numeric only.' });
      return false;
    }

    if (!approvalForm.file_date) {
      fireToast({ type: 'error', message: 'Please choose file date.' });
      return false;
    }

    const remarks = approvalForm.remarks ? approvalForm.remarks.replace(/<\/?[^>]+(>|$)/g, "").replace(/&nbsp;/gi, "").trim() : approvalForm.remarks;
    if (!remarks) {
      fireToast({ type: 'error', message: 'Please enter the remark.' });
      return false;
    }
    ...
    const payload = {
      file_path: file,
      file_number: approvalForm.file_number.trim(),
      file_date: approvalForm.file_date,
      remark: approvalForm.remarks.trim(),
      short_remark: shortRemark
    };
  ...
  submitApproval(module.value.id, approvalForm.draft_id, payload)
    .then(response => {
      if (response.success) {
        fireToast({ type: 'success', message: response.message });
        router.push({ name: 'Leave_Request' });
      } else {
        fireToast({ type: 'error', message: 'Something went wrong!' });
      }
    })
