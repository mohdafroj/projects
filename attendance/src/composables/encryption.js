...
export const fetchPublicKey = async () => {
    ...
    const sdsKey = cookieService.getLocalStorageData({ name: "sdsKey", decode: 1 });
    if (sdsKey && sdsKey != publicKeyPem) {
        publicKeyPem = sdsKey;
    }
    ...
    try {
        ...
        if (res) {
            if (sdsKey && res != sdsKey) {
                cookieService.setLocalStorageData({ name: "sdsKey", value: res, encode: 1 });
...
export async function createEncryptedAxios(instance) {
