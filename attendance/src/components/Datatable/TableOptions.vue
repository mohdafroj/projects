<template>
  <div class="w-full mt-4 rounded-md">
    <div
      class="flex items-center justify-between px-4 py-2.5 bg-white dark:bg-slate-800 rounded-t-md shadow-sm"
    >
    ...
    <div
      ...
      class="space-y-4 bg-white dark:bg-slate-800 mt-0 px-2 py-0 w-full flex flex-row justify-start rounded-b-md"
    >
      <!-- Sort -->
        ...
        >
          ...
          <div class="flex flex-row items-center gap-3 rounded-md">
            <div class="relative flex flex-col w-full">
              <label class="text-xs text-gray-600 dark:text-gray-300 font-semibold mb-1">Add filters</label>
              <select
          ...
          >
            ...
            <div class="flex flex-col gap-4">
              ...
              <Multiselect
                ...
                :placeholder="Boolean(col.multiple) ? 'Select or search...' : 'Click and select a value'"
                ...
                :multiple="Boolean(col.multiple)"
                :searchable="Boolean(col.multiple)"
                :close-on-select="!Boolean(col.multiple)"
...
const handleSearchInput = debounce((key, term) => {
  let colValues = props.columns.filter(item => item.key == key);
  colValues = colValues[0] ? colValues[0]['values'] : [];
  //console.log("search: ", key, term, colValues);
  if (term && term.trim()) {
    let searchKeyword = term.trim();
    if ( colValues.includes(searchKeyword) ) {
      filters[key].push(searchKeyword);
    }
    //filters[key] = [term.trim()];
  } else {
    //if (!Array.isArray(filters[key])) filters[key] = [];
    //else filters[key].splice(0);
  }
  emitFilter();
}, 500);
...
const getUniqueOptions = key => {
  if ( key ) {
    const filterValues = props.columns.filter(item => item.key == key && item.values && Array.isArray(item.values) );
    if ( filterValues.length ) {
      return filterValues[0].values;
    }
  }
