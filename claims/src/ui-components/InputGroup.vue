  ...
  <div :class="`${horizontal ? 'flex' : ''}  ${merged ? 'merged' : ''}`">
    ...
    >
      ...
      >
        ...
        >
          {{ prepend }}
          <Icon :icon="prependIcon" v-if="prependIcon" />
        </span>
      ...
      >
        <span
          ...
          >{{ append }}
          <Icon :icon="appendIcon" v-if="appendIcon" />
        </span>
...
// Method to toggle the type of input
function toggleType() {
...
.input-group-control {
  @apply bg-white dark:bg-slate-900 dark:placeholder:text-slate-400 transition duration-300 ease-in-out border border-gray-300 dark:border-slate-700 focus:ring-0 focus:outline-none placeholder:text-slate-400 text-slate-800 text-base px-3 placeholder:font-light focus:border-gray-600 dark:focus:border-slate-900 dark:text-white;
}

.input-label {
  @apply mb-2 text-slate-500 font-semibold text-[15px] leading-6 capitalize cursor-pointer;
}
...
.input-group-text {
  @apply bg-white dark:bg-slate-900 transition duration-300 ease-in-out flex items-center justify-center px-3 border border-gray-300 dark:border-slate-700 text-slate-800 text-base;
}
...
.inputGroup {
  &.has-prepend-slot {
    .input-group-control {
      @apply border-l-0 focus:ring-0 focus:border-gray-600 dark:focus:border-slate-700;
    }
  }

  &.has-append-slot {
    .input-group-control {
      @apply border-r-0 focus:ring-0 focus:border-gray-600 dark:focus:border-slate-700;
    }
  }
}
...
.append-slot {
  .btn {
    @apply pt-0 pb-0 h-full items-center hover:ring-0;
  }
