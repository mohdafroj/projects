    ...
    >
      ...
      <div class="flex justify-end items-center gap-2">
        ...
        <div class="flex gap-2 items-center">
          <!-- <button @click="goToCGHSForm" class="bg-green-500 rounded
     py-2 px-2 text-white">+Add New</button> -->
          <button
            v-for="(btn, index) in props.addButtons"
            :key="index"
            @click="$emit('addItems', index, btn.text)"
            :class="`${btn.bgColor} ${btn.textColor} ${btn.hoverColor} px-2 py-1 rounded-md text-sm flex items-center hover:opacity-90 `"
          >
          ...
          <div class="flex flex-row items-center gap-3 rounded-md">
            <div class="relative flex flex-col w-full">
              ...
              <select
                ...
                class="border-[1.5px] border-[#d1d5db] text-sm px-8 py-2 rounded appearance-none focus:outline-none w-full"
              ...
              <Icon
                icon="mdi:plus"
                class="absolute left-2 top-[30px] text-lg pointer-events-none"
              />
            </div>
          </div>
...
import { getAllCghsRequestList, getAllMemberList, getAllStatus } from '@/services/rss/cghsServices.js';
...
const dynamicFilterData = ref({});

function goToCGHSForm() {
  router.push('/add-cghs-card/'); // replace with your actual internal route path
}
...
const addFilter = async selectedData => {
  ...
  if (!exists) {     
    await fetchAllCghsRequestList(selectedData); 
...
const fetchAllCghsRequestList  = async field => {
  try {
    isLoading.value = true;
    // for member_cghs card
    if(field==='cghs_card_request_no'){
      const response = await getAllCghsRequestList(); 
      isLoading.value = false;
        if (response.success_code === 200 && Array.isArray(response.data) ) {
        const options = response.data.reduce((acc, item) => { 
          let value = item?.request_no; 
          if (value && value !== '') {
            acc.add(value);
          }
          return acc;
        }, new Set());
        uniqueOptions.value[field] = Array.from(options).sort();
        //console.log('END POINT===>>>',uniqueOptions);
        console.log(
          `Unique options for mprojectmodule ${field}:`,
          uniqueOptions.value[field]
        );
      } else {
        console.warn(
          'Unexpected API response structure for mprojectmodule:',
          response.data
        );
      }
    }
    ...
    if(field==='member_names'){
      const response = await getAllMemberList(); 
     // console.log('ddddddddd',response);
      isLoading.value = false;
        if (response.success_code === 200 && Array.isArray(response.data) ) {
        const options = response.data.reduce((acc, item) => { 
          let value = item?.full_name; 
    ...
    console.error(
      `Error fetching unique options for mprojectmodule ${field}:`,
      error
    );
  }
}
...
//  console.log('UNIQUE OPTIONS==', uniqueOptions.value);
  return uniqueOptions.value[key]||[];
}
...
const onSelectHandle = () => {
  if (selectedField.value) {
    //console.log('here onselect', selectedField.value)
