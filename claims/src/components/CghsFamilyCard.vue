    ...
    <div class="bg-white rounded-lg border-4 border-gray-300 shadow-2xl overflow-hidden ">
      ...
      <div class="p-4 flex gap-4">
        ...
        <div class="flex-1 text-sm space-y-1">
          <p><span class="text-teal-800 font-bold text-sm">NAME:</span>
           <span class="text-teal-800 font-semibold text-sm"> {{ memberData?.full_name }}</span></p>
          <p><span class="font-bold">DOB/GENDER:</span> {{ memberData?.dob }} / {{ memberData?.gender }}
          </p>
          <p><span class="font-bold">Blood Group:</span> 
            <span class="text-teal-800 font-semibold text-sm">{{ memberData?.blood_group ? memberData?.blood_group : '-' }}</span></p>
          <p><span class="font-bold">RELATION:  <span class="text-teal-800 font-semibold text-sm"></span>{{ memberData?.relation ? memberData?.relation : '-' }}</span></p>
          <p><span class="font-bold">Card Status: <span class="text-teal-800 font-semibold text-sm"></span> {{ memberData?.card_status ? memberData?.card_status : 'Pending' }}
          </span></p>
      ...
      <div class="relative h-14 mt-4">
        ...
        <div class="footer-banner flex items-center justify-between px-4" :style="{ background: isActiveCard ? '#D32E2E' : '#848484' }">
          <div class="flex items-center gap-8 text-white ">
            <span class="text-white text-lg font-bold italic"
             ></span>
            <span v-if="memberData?.cghs_number != 9999999">BEN ID : {{ memberData?.cghs_number ?? 'N/A' }}
              &nbsp;</span>
...
const memberData = computed(() => {
  ...
  console.log('hhhhhhhhh', props.requests.family);
