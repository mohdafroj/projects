        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 px-2 pb-2 min-w-[1100px]">
          ...
          <div v-for="col in activeFilters" :key="col.key" class="flex flex-col w-full relative">
            ...
            <div class="flex flex-col gap-4">
              ...
              <Multiselect 
              v-else v-model="filters[col.key]" 
...
watch(
  ...
  cols => {
    if (cols.length > 0) {
      const defaultKey = cols[0].key;
      selectedField.value = defaultKey;

      // Only push if not already in activeFilters
      const exists = activeFilters.value.some(f => f.key === defaultKey);
      if (!exists) {
        activeFilters.value.push({ key: defaultKey, type: cols[0].type });
      }
    }
  },
  { immediate: true },
