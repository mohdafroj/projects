<template>
    <div>
      <span class="text-2xl text-gray-600 mb-4">Session Schedule (Session-267)</span>
      ...
      <Tooltip
        ...
        :position="'top'"
        :width="'auto'"
        :tooltipClass="'bg-gray-900 text-white px-3 py-1 rounded-lg shadow-md'"
        :style="{ position: 'absolute', left: `${tooltipData.position.x}px`, top: `${tooltipData.position.y}px` }"
      >
        <template #default>{{ tooltipData.content }}</template>
      </Tooltip>
    </div>
  </template>
  ...
  import eventsData from "./session-secedule.json";
  ...
  // Handle event hover to show custom tooltip
  const handleEventMouseEnter = (info) => {
    console.log("Hovered over event:", info.event.title, "at", info.jsEvent.clientX, info.jsEvent.clientY);
    
    tooltipData.value = {
      show: true,
      content: info.event.title,
      position: { x: info.jsEvent.clientX + 10, y: info.jsEvent.clientY + 10 } // Offset to avoid overlap
    };
  };
  
  // Hide tooltip when mouse leaves
  const handleEventMouseLeave = () => {
    console.log("Mouse left event, hiding tooltip");
