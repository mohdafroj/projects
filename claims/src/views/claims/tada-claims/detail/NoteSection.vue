<template>
  <div ref="fullscreenElement" class="bg-white rounded-md shadow-sm w-full max-w-max dark:bg-slate-800 dark:text-slate-300" >
    <div class="flex justify-between items-center p-3 border-b">
      <h3 class="text-gray-700 font-semibold text-lg">Add Remarks  </h3>
    ...
    <!-- Notes List -->
    <div class="px-4  dark:bg-slate-800 pb-4 space-y-3" v-if="paginationData.length">
      <!-- Note 1 -->
      <div
        v-for="note in paginationData"
        :key="note.comm_id"
        :class="[
          note.self_note ? 'bg-orange-100' : 'bg-green-50', 'dark:bg-slate-700',
      ...
      >
        ...
        <div class="mt-2 text-right">
          <div class="text-xs">{{ note.actor_name }}</div>
          <div class="text-xs">{{ note.actor_designation }}</div>
          <div class="text-xs">{{ useLocalDate(note.created_at) }}</div>
        </div>
      </div>
    </div>
...
import {addNote, updateNote} from "@/services/rss/TadaServices";
import Swal from 'sweetalert2';
import { PERMISSIONS, hasPermission } from "@/utils/rbac";
import { useApiStore } from "@/store/apiData";
import PaginationSelect from "/src/views/claims/it-equipment/PaginationSelect.vue";
...
const contentNot = computed(() => {
  let status = true;
  let claimStatus = apiStore.tada_claim_store.detail.status.toLowerCase() || '';
  const assignedTo = apiStore.tada_claim_store.detail.assigned_to || 0;
  if ( hasPermission([PERMISSIONS.ITCLAIM.APPROVE])) {
    ...
    if ( (assignedTo == currentUserId) && ['submitted','initiated'].includes(claimStatus) ) {
      status = false;
    }
...
let notes = (apiStore.tada_claim_store.detail?.notes || []).map(note => ({
    ...note,
...
watch(
  () => apiStore.tada_claim_store.detail?.notes,
  (newData) => {
    if (newData) {
...
const findItemDetails = () => {
  const claimItems = apiStore.tada_claim_store.detail?.claim_items || [];
  const touchedItems = apiStore.tada_claim_store.detail?.touched_items || [];
...
const addNoteRemark = async (self_note=0) => {
  ...
  findItemDetails();
  const payload = {
    claim_id: apiStore.tada_claim_store.detail.claim_id,
  ...
  if ( response.isError == false ) {
    if ( response.success_code == 200) {
      ...
      apiStore.setItEquipment({...apiStore.tada_claim_store, detail: {...apiStore.tada_claim_store.detail, touched_items:[], notes:claimNotes.value}});
...
const publishRemark = async (id) => {
  ...
  if ( ! response.isError ) {
    ...
    apiStore.setItEquipment({...apiStore.tada_claim_store, detail: {...apiStore.tada_claim_store.detail, notes:claimNotes.value}});
  }
  noteSwalPopup({isError: response.isError, message});
}
