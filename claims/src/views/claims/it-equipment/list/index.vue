  ...
  <div v-else class="table-container">
    ...
    >
      <div class="flex border-b mt-2 bg-slate-300 p-2">
        ...
        <button @click="() => selectTab('all')" :class="selectedTab == 'all' ? 'bg-white' : 'hover:text-slate-900'" class="rounded-[4px] px-2 font-medium ml-1 text-slate-500" type="button">All Claims</button>
      </div>     
      <CustomTable 
        :data="pagedData" 
...
const tableHeading = computed( () => t("menu.it_claim") + " " + t("menu.claims"));
...
const buttonMenu = computed(() => {
  let result = [];
  if(hasPermission([PERMISSIONS.ITCLAIM.INITIATE])) {
    result = [
      {key:'add', text:'Add Claim', textColor:'underline', bgColor: '', hoverColor: '', icon:'mdi:plus'}
    ];
  }
...
const fetchClaimList = async () => {
    ...
    if ( selectedTab.value == 'assigned' ) {
      claimListOptions.params = {...claimListOptions.params, type:'IT-EQUIPMENT', card_type: selectedCard.value, tab: selectedTab.value };
    } else {
      claimListOptions.params = {...claimListOptions.params, type:'IT-EQUIPMENT', card_type: selectedCard.value};
    }
    pagedDataLoader.value = true;
    pagedData.value = [];
    claimPagination.value = {current_page:0, last_page:0};
    const response = await claimList(claimListOptions);
