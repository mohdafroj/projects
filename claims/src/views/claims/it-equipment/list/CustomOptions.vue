      ...
      <div class="flex justify-end items-center gap-2">
        ...
        <div class="flex gap-2 items-center">
          <button
            ...
            :key="index"
            @click="$emit('addItems', btn)"
            :class="`${btn.bgColor} ${btn.textColor} ${btn.hoverColor} px-4 py-2 text-sm flex items-center hover:opacity-90`"
          >
            <Icon v-if="btn.icon" :icon="btn.icon" class="text-base mr-1" />
            {{ btn.text }}
          </button>
        </div>
        <!-- Filter Icon -->
              ...
              <template v-if="getFieldType(col.key) === 'Date'">
                <div class="relative">
                  ...
                  <div
                    ...
                    @focusout="(e) => handleCalendarClickOutside(e, col.key)"
                  >
              ...
              <template v-else-if="getAmountFieldType(col.key) === 'Amount'">
                <div class="relative">
                  ...
                  <div
                    ...
                    class="absolute z-10 mt-1 w-full bg-white shadow-lg rounded-md border border-gray-200 py-2 px-3 space-y-2"
                    @focusout="(e) => handleAmountClickOutside(e, col.key)"
                  >
                    <div class="flex items-center justify-between">
                      <label class="text-sm text-gray-700">From:</label>
                      <input
...
const handleAmountClickOutside = (e, key) => {
  //activeAmount.value = "";
  const nextFocus = e.relatedTarget;
  if (nextFocus && nextFocus.type === "number") {
    return;
  }
  if ( activeAmount.value && filters[key]?.[0] && filters[key]?.[1]) {
    activeAmount.value = null;
    emitFilter();
  } else if ('dateFilter' == activeAmount.value) {
    activeAmount.value = null;
  }
}

const handleCalendarClickOutside = (e, key) => {
  const nextFocus = e.relatedTarget;
  if (nextFocus && nextFocus.type === "date") {
    return;
  }
  if ( 'dateFilter' == activeCalendar.value && filters[key]?.[0] && filters[key]?.[1]) {
    activeCalendar.value = null;
    emitFilter();
  } else if ('dateFilter' == activeCalendar.value) {
    activeCalendar.value = null;
  }
}
...
const handleSearchInput = debounce((key, term) => {
  ...
  const columnValues = props.columns?.find(col => col.key === key)?.values;
  if (columnValues && Array.isArray(columnValues)) {
    const findTerm = columnValues.find(item => item.toLowerCase() === searchTerm.toLowerCase() );
    if (findTerm) {
      filters[key] = [findTerm];
    }
  } else {
    if (searchTerm) {
...
const getUniqueOptions = key => {
  let checkValues = props.columns.filter(item => item.key == key);
  if ( checkValues[0] && checkValues[0]['values'].length ) {
    return checkValues[0]['values'];
  }
